...
namespace TrilinosWrappers
{
  ...
  public:
    ...
    */
    void compress (const Tpetra_CombineMode last_action) DEAL_II_DEPRECATED;
...
// ------------------- inline and template functions --------------
  ...
  VectorBase::locally_owned_elements() const
  {
    ...
    else if (vector->getMap().getNodeNumElements() > 0)
      {
        const size_type n_indices = vector->getMap().getNodeNumElements();
        Teuchos::ArrayView<global_dof_index> vector_indices = vector->getMap().getNodeElementsList();
        is.add_indices(vector_indice.getRawPtr(), vector_indices+n_indices);
        is.compress();
      }
  ...
  inline
  void
  VectorBase::compress (const Tpetra::CombineMode last_action)
  {
