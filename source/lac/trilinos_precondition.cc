...
#if DEAL_II_TRILINOS_VERSION_GTE(11,12,0)
...
//  PreconditionAMGMueLu::AdditionalData::AdditionalData (const bool symmetric,
//                                                        const std::string multigrid_algorithm,
//                                                        const bool w_cycle,
//                                                        const std::vector<std::vector<bool> > &constant_modes,
//                                                        const unsigned int smoother_sweeps,
//                                                        const unsigned int smoother_overlap,
...
// :
//   symmetric(symmetric),
//   multigrid_algorithm(multigrid_algorithm),
//   w_cycle(w_cycle),
//   constant_modes(constant_modes),
//   smoother_sweeps(smoother_sweeps),
//   smoother_overlap(smoother_overlap),
...
//  {}
                                    ...
                                    const AdditionalData   &additional_data)
  {
    ...
    initialize (matrix, parameter_list);
//------------------------------------------------------------------------------------
//    // The following code creates a preconditioner using the factory
//    // functions.
//
//    preconditioner.reset ();
//
...
//
//    // Turns matrix into a MueLu::Matrix. The constness needs to be cast away.
//    // MueLu onlyd Teuchos::RCP which are Trilinos version of std::shared_ptr.
//    Teuchos::RCP<Epetra_CrsMatrix> rcp_matrix = Teuchos::rcpFromRef(
//        *(const_cast<Epetra_CrsMatrix*>(&matrix)));
//    // This is very similar MueLu::CreateEpetraOperator because the
//    // function does not play with preconditioner.reset()
//    // We cannot use MueLu::CreateEpetraOperator because of the
//    // preconditioner.reset().
...
//
//    Teuchos::RCP<MueLu::SmootherPrototype<double,int,int,node> > smoother_prototype;
//    std::string ifpack_type;
//    Teuchos::ParameterList ifpack_list;
//
//    if (additional_data.smoother_type.compare("Chebyshev")==0)
//    {
//      ifpack_type = "Chebyshev";
//
//      // Compute the largest eigenvalue.
//      double max_eigenvalue,min_eigenvalue;
//      Epetra_Vector inv_diag(matrix.Map());
//      matrix.ExtractDiagonalCopy(inv_diag);
//      Ifpack_Chebyshev::CG(matrix,inv_diag,5,min_eigenvalue,max_eigenvalue);
//      ifpack_list.set("chebyshev: max eigenvalue", max_eigenvalue);
//      ifpack_list.set("chebyshev: ratio eigenvalue", 10);
//      ifpack_list.set("chebyshev: degree", static_cast<int>(additional_data.smoother_sweeps));
//    }
//    else if  ((additional_data.smoother_type.compare("Gauss-Seidel")==0) ||
//        (additional_data.smoother_type.compare("symmetric Gauss-Seidel")==0))
//    {
//      ifpack_type = "point relaxation stand-alone";
//      ifpack_list.set("relaxation: type", additional_data.smoother_type);
//      ifpack_list.set("relaxation: sweeps", static_cast<int>(additional_data.smoother_sweeps));
//    }
//    else
//      ExcNotImplemented();
//
//    smoother_prototype = Teuchos::rcp(new MueLu::IfpackSmoother(ifpack_type,ifpack_list));
//    Teuchos::RCP<MueLu::SmootherFactory<double,int,int,node> > smoother_factory;
//    smoother_factory = Teuchos::rcp(new MueLu::SmootherFactory<double,int,int,node>(smoother_prototype));
//
//    // Create coarse solver
//    Teuchos::RCP<MueLu::SmootherPrototype<double,int,int,node> > coarse_solver_prototype;
//    // Direct Solver
//    std::string coarse_solver_type = additional_data.coarse_type;
//    Teuchos::ParameterList coarse_solver_list;
//    coarse_solver_prototype = Teuchos::rcp(new MueLu::DirectSolver<double,int,int,node> (
//          coarse_solver_type,coarse_solver_list));
//    Teuchos::RCP<MueLu::SmootherFactory<double,int,int,node> > coarse_solver_factory =
//      Teuchos::rcp(new MueLu::SmootherFactory<double,int,int,node>(coarse_solver_prototype,
//            Teuchos::null));
//
//    // Multigrid Hierarchy
//    Teuchos::RCP<MueLu::Hierarchy<double,int,int,node> > hierarchy =
//      Teuchos::rcp(new MueLu::Hierarchy<double,int,int,node>(muelu_matrix));
//
//    // Does not work...
//    hierarchy->setVerbLevel(Teuchos::VERB_NONE);
//
//    // Multigrid setup phase
//    MueLu::FactoryManager<double,int,int> manager;
//    manager.SetFactory("Smoother", smoother_factory);
//    manager.SetFactory("CoarseSolver", coarse_solver_factory);
//    hierarchy->Setup(manager);
//
...
//    Teuchos::ParameterList parameter_list;
//
//    parameter_list.set("problem: symmetric",additional_data.symmetric);
//    parameter_list.set("multigrid algorithm",additional_data.multigrid_algorithm);
//
//    if (additional_data.smoother_type.compare("Chebyshev")==0)
//    {
...
//      Ifpack_Chebyshev::CG(matrix,inv_diag,5,min_eigenvalue,max_eigenvalue);
//
//    //  parameter_list.set("chebyshev: max eigenvalue", max_eigenvalue);
//    //  parameter_list.set("chebyshev: ratio eigenvalue", 10);
//    //  parameter_list.set("chebyshev: degree", static_cast<int>(additional_data.smoother_sweeps));
//    }
//    else if  ((additional_data.smoother_type.compare("Gauss-Seidel")==0) ||
//        (additional_data.smoother_type.compare("symmetric Gauss-Seidel")==0))
//    {
//      parameter_list.set("smoother: type","RELAXATION");
//      parameter_list.set("relaxation: type", additional_data.smoother_type);
//      parameter_list.set("relaxation: sweeps", static_cast<int>(additional_data.smoother_sweeps));
//    }
//    else
//      ExcNotImplemented();
//
...
//
//    parameter_list.set("smoother: overlap",
//                       static_cast<int>(additional_data.smoother_overlap));
//    parameter_list.set("aggregation: threshold",
//                       additional_data.aggregation_threshold);
//
//    if (additional_data.output_details)
//    {
//      parameter_list.set("verbosity", "extreme");
//      parameter_list.set("print unused parameters",true);
//    }
//    else
//    {
//      parameter_list.set("verbosity", "none");
//      parameter_list.set("print unused parameters",false);
//    }
//
//    const Epetra_Map &domain_map = matrix.OperatorDomainMap();
//
